...
## 2026-09-02: ADB 重写为 RFT 力基波驱动模型（答辩课题 ①）

### 背景

用户对旧 ADB（路径跟随脊柱 + 装饰波 + base_step 公式）的核心质疑:**波驱动到底驱动了什么?** 结论——旧模型里波只是装饰,推进 = 头部步长(被人造线性公式调制),波参数对运动无因果。用户追问"真实物理里前进方向沿合力方向"促使全面重做。答辩课题三块结构(①波推进修复/②身体感知通道/③参数控制适用边界)见记忆 [[thesis-three-part-plan]]。

### 变更内容

**1. ADB = RFT 力基波驱动(`worm_body.py` 类整体重写)**
- 身体局部坐标系:质心 com + 身体轴朝向 body_theta + 形状 y(s) = A·env(s)·sin(ks−φ) + (b/2)s²
- 每子步 **3×3 力/力矩平衡**解 (Vx, Vy, Ω):每点速度 = V + Ω×r + 行波横向速度;各向异性阻力 F = −D·v;自推进条件 ΣF=0, Σr×F=0
- **速度与转向都是涌现量**:波 → 推力(方向与波传播相反),曲率偏置 b → 净力矩 → 身体旋转(b 符号定左右)
- **无 heading 动作**:AC 动作 = (波幅, 频率, 曲率偏置),ADB 仅支持 Actor-Critic
- **能量 = 机械耗散功** ΣvᵀDv·Δs·dt(替换拍脑袋系数 0.12×距离 等)
- **子步积分**:决策步 ≠ 物理步,每决策步 sub_steps=10 个子步;history 每子步一帧(动画平滑行波)
- **波幅包络** env(s)=sin(π(s+L/2)/L):头尾幅值为零(生物真实;削弱端点力矩伪影)
- **中点采样**:n 点放 n 等分段中心(端点采样会在 λ=L 时对同一相位重复计数,产生与点数无关的横向漂移伪影)
- drag_ratio **固定 1.4**(C. elegans 实测法向/切向阻力比,非可调);drag_coeff 只缩放能耗不影响速度(速度与粘度无关,Taylor 结果)
- 感知位置 = 质心;reset_worm_for_new_round 的直线段经 `_sync_centerline_from_public_segments` 覆写解释为新一轮初始轴

**2. DDPG 泛化(`actor_critic.py`)**
- Actor:任意 action_dim + 每维 (lo,hi) 仿射映射 `(tanh+1)/2·(hi−lo)·scale+lo`,可学习 scale
- DDPGAgent:action_bounds 按维裁剪;CCB 默认 [(-π,π),(0.05,5.0)] 输出范围不变

**3. 课程两阶段训练**
- `curriculum_freeze_steps`:前 N 个决策步强制 b=0(只学速度-能耗子问题),之后解锁 b 学转向;`_build_actor_action` 内实现,零引擎改动

**4. UI(`app.py`)**
- ADB:动作空间固定"连续波参数",算法仅 Actor-Critic;新增子步数/阻力系数/课程步数/包络开关;移除无意义的步长/转向角/刚度滑块

**5. 物理离线验证(`verify_rft_physics.py`,可重复运行)**
- [1] 方向:波向尾传 → 身体前向 ✓;**侧漂角 −5.1°** = 有限线虫相位锁定平均倾角(端点力矩),镜像协变验证通过(相位 π 出发漂移精确反号)——真实物理非 bug
- [2] Taylor 平方律:速度 ∝ A²·f,R²=0.994 ✓
- [3] 各向同性 drag_ratio=1.0 → 推进归零 ✓(各向异性是必要条件)
- [4] 曲率偏置 b=±0.03 → ±0.19°/步 双向转向 ✓
- 输出 A-f 地形图 PNG(rft_verify_output/)

### 过程中发现并处理的 bug

| bug | 处理 |
|---|---|
| 端点采样 λ=L 时首尾点相位重合 → 横向漂移伪影(与点数无关) | 中点采样 |
| `_initialize_centerline` 无条件重置 wave_phase,吞掉 body_param | 尊重传入值(轮次重置在 sync 覆写中处理) |
| neural_networks.py 缺 UTF-8 stdout 重配置(GBK 下 ✓ 打印崩溃) | 与 app/utils 同款修复 |
| 测试测量窗口 30 步非整周期 → 身体轴摆动(±18°)残留污染 | 整周期窗口(4 宏步/周期) |

### 验证

```
pytest:        40/40 (31 原有 + 9 新增: Taylor 缩放 / 各向同性 / 双向转向 / 镜像协变 /
               子步+能耗 / AC 3 维+课程冻结 / AC-only 报错+冒烟 / 轮次重置 / 引擎级 ADB+AC)
test_simple:   全部通过
py_compile:    通过
verify_rft_physics: 4/4 PASS + 地形图
```

### 下一步

1. **① 的实证**:Streamlit 跑 ADB+AC(课程两阶段),观察收敛后的巡航/冲刺/省电三档速度行为与 b 转向策略;A-f 效率地形图入论文
2. **② 身体感知通道**:奖励加身体平均温度,检验等温线追踪涌现
3. 反转/omega 转向(RFT 框架下 = 短时大 b + 波向反转,动作空间扩展)
4. 能量耗尽终止(done 信号)

---

## 2026-09-09: ADB+AC 训练实证 — 收敛但慢,诊断分析(未改代码)

### 训练观察(用户报告)

- 奖励曲线**先上升、后收敛到中间值**
- 身体确实扭动前进(波驱动生效 ✓)
- **速度太慢**:一个 episode 的步数内只移动一点点
- 奖励变体为 original(未开能量,按用户要求先不管)

### 诊断数据(快速诊断脚本实测)

速度表(格/决策步):A=1.0,f=0.25 → 0.065;A=1.3,f=0.5 → 0.20;A=2.0,f=0.8(上限) → **0.60**。
曲率违例率:**A≤1.8 全部 0%**(波幅包络使波形温和),A=2.0 才 7.1% → 违例惩罚几乎不绑定。
疲劳:不进奖励(`compute_reward` 无疲劳项,已核对)。

### 结论:慢是训练动力学问题,不是物理限制

- 物理上没有任何东西阻止 A·f 推到上限——0.6 格/步无惩罚可及,策略却没去够
- **鸡生蛋循环**:速度慢 → episode 内到不了暖区 → "更快"不产生奖励差异 → A·f 方向梯度≈0 → 策略冻结在探索恰好留下的中等值 → 继续慢
- 曲线"先升后落到中间值" = DDPG 标准动力学(噪声探索期上升 → 噪声衰减回落 → 确定性策略真实值平台)
- 配套因素:γ=0.95 折扣削弱"早到"价值;3 维连续动作 + 几百步训练预算偏少;OU 噪声衰减快(0.9995/步)
- **能量变体单独打开不会变快**(只惩罚高 A·f,会让策略更慢)——它要等速度尺度修好后再开,提供"省电档"权衡信号

### 修复杠杆(按优先级,待用户决定后实施)

1. **λ 12→6~8**:速度 ×2~3(公式 U∝A²·k·ω,k 变大;最小改动最大效果)
2. **延长 episode / 缩小场地**:让"到达暖区"在 episode 内可发生——速度梯度存在的前提
3. **γ 0.95→0.99**:让"早点到"更值钱
4. 训练预算加大
5. 诊断先行:训练时记录每轮 (A,f,b) 均值/速度/距最佳点距离,数据确认上述假设后再改

### 其他

- 新增 `程序/启动Streamlit.bat`(双击启动,免命令行)
- 系统曾因内存不足杀掉后台 Streamlit 进程(机器内存偏紧,PyTorch 训练与 Streamlit 同开需注意)
